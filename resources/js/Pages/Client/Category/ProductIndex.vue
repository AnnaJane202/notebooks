...
export default {
    ...
    methods: {
        setFilter(param, value) { //это только для чекбокса
            console.log(param);
            console.log(value);
        ...
        setFilterPopular(param) { //это только для чекбокса popular
    ...
    <div class="hidden md:block bg-pink-700 p-4 text-white">
        <div class="flex place-content-around ">
            <template v-for="category in categories">
                <Link v-if="category.title !== this.category.title"  :href="route('client.categories.products.index', category.id)" class="inline hover:scale-125 transition-transform duration-300 ease-in-out">{{ category.title }}</Link>
                <Link v-if="category.title === this.category.title"  :href="route('client.categories.products.index', category.id)" class="inline hover:scale-125 transition-transform duration-300 ease-in-out italic text-gray-400">{{ category.title }}</Link>
            </template>
...
<!--                <h2 class="text-xl font-semibold mb-4">Наши товары</h2>-->
    ...
    <div class=" hidden container mx-auto">
        ...
        <div class="flex">
            <div @click="open = true" class="md:hidden py-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-9 text-pink-600 cursor-pointer hover:text-pink-800">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                </svg>
            </div>
            <div class="m-4">
                <Link :href="route('client.products.index')" class="hover:text-pink-600">На главную</Link>
                <span class="text-indigo-600"> > {{ category.title }}</span>
            </div>
                                        ...
                                        <div class="flex h-full flex-col overflow-y-auto bg-white py-6 shadow-xl">
                                            ...
                                            <div class="relative mt-6 flex-1 px-4 sm:px-6">
                                                <div class=" p-4">
                                                    <div class="flex flex-col place-content-around" >
                                                        <template v-for="category in categories">
                                                            <Link v-if="category.title !== this.category.title"  :href="route('client.categories.products.index', category.id)" class="inline text-lg md:text-xl border-b border-b-pink-200 hover:text-pink-600 p-5">{{ category.title }}</Link>
                                                            <Link v-if="category.title === this.category.title"  :href="route('client.categories.products.index', category.id)" class="inline text-lg md:text-xl border-b border-b-pink-200 hover:text-pink-400 p-5 italic text-pink-600">{{ category.title }}</Link>
                                                        </template>
                                                    </div>
                                                </div>
                                            </div>
